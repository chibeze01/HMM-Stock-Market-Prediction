...
def rolling_directional_accuracy(
    ...
    # ⚡ Bolt: Fast array mapping instead of np.vectorize(dict.get)
    flat_states = hidden_states.flatten()
    max_map_key = max(direction_map.keys()) if direction_map else 0
    max_state = max(max_map_key, flat_states.max() if flat_states.size > 0 else 0)

    mapping_arr = np.zeros(max_state + 1, dtype=int)
    for state, direction in direction_map.items():
        mapping_arr[state] = direction

    predicted_direction = mapping_arr[flat_states]
