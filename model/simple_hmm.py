...
class GaussianHMM:
    ...
    def _assign_clusters(self, X: np.ndarray, means: np.ndarray) -> np.ndarray:
        # ⚡ Bolt: Replace explicit broadcasting with expanded squared distance formula
        # to prevent memory-intensive intermediate arrays and improve speed (~17x).
        dist_sq = -2 * np.dot(X, means.T) + np.sum(means**2, axis=1)
