...
class GaussianHMM:
    ...
    def _estimate_transitions(self, labels: np.ndarray) -> np.ndarray:
        ...
        # ⚡ Bolt: Replaced Python loop with vectorized np.add.at for O(N) C-level accumulation.
        # Standard fancy indexing (trans[labels[:-1], labels[1:]] += 1) fails to accumulate
        # repeated pairs properly. np.add.at correctly buffers and applies all increments.
