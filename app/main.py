...
if st.session_state["model"] is None:
    ...
    with eval_tabs[1]:
        if evaluation.rolling_accuracy.empty:
            st.info(
                "**No Rolling Accuracy Available**\n\n"
                "Rolling accuracy series will appear once enough data is available. "
                "Try extending the date range or selecting a different stock.",
                icon="📈",
            )
        else:
    ...
    with eval_tabs[2]:
        if evaluation.rolling_log_likelihood.empty:
            st.info(
                "**No Rolling Log Likelihood Available**\n\n"
                "Rolling log-likelihood requires additional observations. "
                "Try extending the training window in the sidebar.",
                icon="📊",
            )
        else:
