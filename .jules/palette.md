...
## 2026-04-19 - Structure component-level empty states
**Learning:** Component-level empty states (like missing data for evaluation charts) are often overlooked. Upgrading generic single-line instructions to include a bold header, actionable next steps, and a relevant icon transforms them from simple errors into helpful user guidance that reduces frustration.
**Action:** Always structure component-level empty states using explicit bold headers and actionable next steps (using implicit string concatenation for cleanliness) along with relevant icons to improve visual hierarchy and readability.
