...
## 2024-05-27 - Vectorizing Backtest Simulation Loop
**Learning:** Simulating trades row-by-row in Python space using a `for` loop over time-series arrays acts as a performance bottleneck.
**Action:** Use vectorized operations like `np.diff` on state/signal arrays to identify entry and exit indices in O(N) C-level operations. Then, use arrays to calculate prices, returns, and `np.cumprod` for capital evolution. Finally, piecewise assignment can be used to construct step-functions like equity curves.
